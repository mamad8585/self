...
local function bot_stats()
  ...
  local text = '✅ کاربرانی که به من پیام فرستادند : '..r

  hash = 'chat:*:users'
  r = redis:eval(redis_scan, 1, hash)
  text = text..'\n✅ کل گروهای که در آن ها عضوم : '..r
  return text
end
local function run(msg, matches)
  if matches[1]:lower() == 'هیدن' then -- Put everything you like :)
    local about = _config.about_text
    local name = user_print_name(msg.from)
    savelog(msg.to.id, name.." ["..msg.from.id.."] used /hidden ")
    return about
  end 
  ...
  if matches[1]:lower() == "امار" then
    if not matches[2] then
      ...
      if msg.to.type == 'چت' or msg.to.type == 'channel' then
    ...
    if matches[2] == "هیدن" then -- Put everything you like :)
