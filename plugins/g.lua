...
local function check_member_super(cb_extra, success, result)
  ...
  for k,v in pairs(result) do
    ...
    if member_id ~= our_id then
      ...
      save_data(_config.moderation.data, data)
	  local text = '✅انجام شد\nℹ️سوپر گروه به لیست سوپر گروه ها اضافه شد'
...
local function check_member_superrem(cb_extra, success, result)
  ...
  for k,v in pairs(result) do
    ...
    if member_id ~= our_id then
      ...
      save_data(_config.moderation.data, data)
	  local text = '✅انجام شد\nℹ️سوپر گروه از لیست سوپر گروه ها حذف شد'
      return reply_msg(msg.id, text, ok_cb, false)
    end
  end
end
...
local title ="ℹ️اطلاعات برای سوپر گروه: ["..result.title.."]\n\n"
local admin_num = "*⃣تعداد ادمین ها: "..result.admins_count.."\n"
local user_num = "🎴تعداد کاربران: "..result.participants_count.."\n"
local kicked_num = "⛔️تعداد افراد کیک شده: "..result.kicked_count.."\n"
...
--vardump(result)
local text = "❎تعداد افراد کیک شده برای این سوپرگروه"..cb_extra.receiver.."\n\n"
local i = 1
for k,v in pairsByKeys(result) do
if not v.print_name then
	name = " "
...
local function lock_group_links(msg, data, target)
  ...
  if group_link_lock == 'بلی' then
    return '✅انجام شد\n✳️ارسال لینک باز شد'
  else
    data[tostring(target)]['settings']['lock_link'] = 'بلی'
    save_data(_config.moderation.data, data)
    return '✅انجام شد\n✳️ارسال لینک قفل شد'
  end
end
...
local function unlock_group_links(msg, data, target)
  ...
  if group_link_lock == 'خیر' then
    return '❎خطا\n🔂ارسال لینک قفل نشده بود'
  else
    data[tostring(target)]['settings']['lock_link'] = 'خیر'
    save_data(_config.moderation.data, data)
    return '❎خطا\n🔁ارسال لینک از قبل فعال بود'
  end
end
...
local function lock_group_all(msg, data, target)
  ...
  if group_all_lock == 'بلی' then
    return '✅خطا\nⓂ️همه ی تنظیمات قفل شده بود'
  else
    data[tostring(target)]['settings']['all'] = 'بلی'
    save_data(_config.moderation.data, data)
    return '✅انجام شد\nⓂ️همه ی تنظیمات قفل شد'
  end
end
...
local function unlock_group_all(msg, data, target)
  ...
  local group_all_lock = data[tostring(target)]['settings']['all']
  if group_all_lock == 'خیر' then
    return '✅خطا\nⓂ️همه ی تنظیمات قفل نشده بودند'
  else
    data[tostring(target)]['settings']['all'] = 'خیر'
    save_data(_config.moderation.data, data)
    return '✅انجام شد\nⓂ️همه ی تنظیمات باز شد'
  end
end
...
local function modlist(msg)
  ...
  local i = 1
  local message = '\nⓂ️لیست مدیران برای ' .. string.gsub(msg.to.print_name, '_', ' ') .. ':\n'
...
local function callbackres(extra, success, result)
  ...
  elseif get_cmd == "دعوت" then
	...
	elseif get_cmd == "مقام" then
		local receiver = extra.channel
		local user_id = result.peer_id
		--local user = "user#id"..result.peer_id
		promote2(receiver, member_username, user_id)
		--channel_set_mod(receiver, user, ok_cb, false)
	elseif get_cmd == "کسر مقام" then
		local receiver = extra.channel
		local user_id = result.peer_id
		local user = "user#id"..result.peer_id
		demote2(receiver, member_username, user_id)
	...
	elseif get_cmd == 'موت کاربر' then
		local user_id = result.peer_id
		local receiver = extra.receiver
		local chat_id = string.gsub(receiver, 'channel#id', '')
		if is_muted_user(chat_id, user_id) then
			unmute_user(chat_id, user_id)
			send_large_msg(receiver, " ["..user_id.."] ان موت شد✅")
		elseif is_owner(extra.msg) then
			mute_user(chat_id, user_id)
			send_large_msg(receiver, " ["..user_id.."] موت شد✅")
		end
	end
end
...
local function in_channel_cb(cb_extra, success, result)
  ...
  if member then
    text = 'کاربری با یوزرنیم @'..member..' در این سوپر گروه وجود ندارد❎'
  else
    text = 'کاربری با شناسه ['..memberid..'] در این سوپرگروه وجود ندارد❎'
  end
    ...
    if vusername == member or vpeer_id == memberid then
     ...
     local sender = cb_extra.msg.from.id
      ...
      if is_momod2(user_id, channel_id) and not is_admin2(sender) then
        return send_large_msg("channel#id"..channel_id, "شما نمی توانید ادمین هارا کیک کنید❎")
      end
...
local function run(msg, matches)
	...
	elseif msg.to.type == 'channel'then
		if matches[1] == 'سوپر' then
			...
			return "❎خطا\n💠شما نمی توانید سوپر گروه را دوباره تبدل کنید!"
		end
	end
	...
	local name_log = print_name:gsub("_", " ")
	local data = load_data(_config.moderation.data)
		if matches[1] == 'mark' and not matches[2] then
			if not is_admin1(msg) and not is_support(support_id) then
				return
			end
			if is_super_group(msg) then
				return reply_msg(msg.id, '❎خطا\n💠سوپر گروه از قبل اد شده بود', ok_cb, false)
			end
		...
		if matches[1] == 'unmark' and is_admin1(msg) and not matches[2] then
			if not is_super_group(msg) then
				return reply_msg(msg.id, '❎خطا\n🔂سوپر گروه اد نشده است!', ok_cb, false)
			end
		...
		if matches[1] == 'makelink' and is_momod(msg)then
		...
		if matches[1] == 'link' then
			...
			if not group_link then
				return "این گروه لینک ندارد 🗣\nبرای ثبت لینک ثبت لینک و برای ساخت لینک لینک جدید را بفرستید ✅"
			end
		...
		--[[if matches[1] == 'حذف' and is_momod(msg) then
			...
			elseif matches[1] == 'ادمین' and not string.match(matches[2], '^%d+$') then
				...
				local username = matches[2]
				local username = string.gsub(matches[2], '@', '')
				savelog(msg.to.id, name_log.." ["..msg.from.id.."] را ادمین کرد🗣 @"..username)
				resolve_username(username, callbackres, cbres_extra)]]
				local	get_cmd = 'ادمین'
				local	msg = msg
				local username = matches[2]
		...
		if matches[1] == 'مقام' then
		  if not is_momod(msg) then
				return
			end
			if not is_owner(msg) then
				return "فقط صاحب/ادمین می تواند این کار را انجام دهد 🗣"
			end
			if type(msg.reply_id) ~= "nil" then
				local cbreply_extra = {
					get_cmd = 'مقام',
			...
			elseif matches[1] == 'مقام' and string.match(matches[2], '^%d+$') then
				local receiver = get_receiver(msg)
				local user_id = "user#id"..matches[2]
				local get_cmd = 'مقام'
				savelog(msg.to.id, name_log.." ["..msg.from.id.."] promoted user#id"..matches[2])
				user_info(user_id, cb_user_info, {receiver = receiver, get_cmd = get_cmd})
			elseif matches[1] == 'مقام' and not string.match(matches[2], '^%d+$') then
				local cbres_extra = {
					channel = get_receiver(msg),
					get_cmd = 'مقام',
				}
		...
		if matches[1] == 'کسر مقام' then
			...
			elseif matches[1] == 'کسر مقام' and string.match(matches[2], '^%d+$') then
				local receiver = get_receiver(msg)
				local user_id = "user#id"..matches[2]
				local get_cmd = 'کسر مقام'
		...
		if matches[1] == "تغیر نام" and is_momod(msg) then
			local receiver = get_receiver(msg)
		...
		if matches[1] == "تغیر درباره" and is_momod(msg) then
			local receiver = get_receiver(msg)
			...
			return "✅درباره ثبت شد\n⬇️می توانید با زدن روی چت درباره را ببینید"
		end
		...
		if matches[1] == 'تغیر قوانین' and is_momod(msg) then
			rules = matches[2]
			local target = msg.to.id
		...
		if msg.media then
			if msg.media.type == 'عکس' and data[tostring(msg.to.id)]['settings']['set_photo'] == 'waiting' and is_momod(msg) then
		...
		if matches[1] == 'تغیر عکس' and is_momod(msg) then
			...
			return 'عکس جدید گروه رو ارسال کن 💠'
		end

		if matches[1] == 'ریست' then
			if not is_momod(msg) then
				return
			end
			if not is_momod(msg) then
			...
			if matches[2] == 'موت ها' then
				chat_id = msg.to.id
				local hash =  'mute_user:'..chat_id
					redis:del(hash)
				return "لیست موت پاک شد✅"
			end
		...
		unlock_group_operator(msg, data, target),
      	...
      	return lock_group_etehad(msg, data, target), etehad
			...
			if matches[2] == 'افزودن ممبر' then
				savelog(msg.to.id, name_log.." ["..msg.from.id.."] locked member ")
				return lock_group_membermod(msg, data, target)
			end		    
			if matches[2]:lower() == 'سایت' then
		...
		if matches[1] == 'موت' and is_owner(msg) then
			local chat_id = msg.to.id
			if matches[2] == 'صدا' then
			local msg_type = 'صدا'
				if not is_muted(chat_id, msg_type..': بلی') then
					savelog(msg.to.id, name_log.." ["..msg.from.id.."] set SuperGroup to: mute "..msg_type)
					mute(chat_id, msg_type)
					return msg_type.." بسته شد🔈"
				else
					return "قفل  "..msg_type.." از قبل فعال بود🔈"
				end
			end
			...
			if matches[2] == 'پرونده' then
			local msg_type = 'پرونده'
				if not is_muted(chat_id, msg_type..': yes') then
					savelog(msg.to.id, name_log.." ["..msg.from.id.."] set SuperGroup to: mute "..msg_type)
					mute(chat_id, msg_type)
					return msg_type.." بسته شد🔈"
		...
		if matches[1] == 'باز' and is_momod(msg) then
			...
			local msg_type = 'صدا'
				if is_muted(chat_id, msg_type..': yes') then
					...
					return msg_type.." باز شد🔈"
				else
					return "موت "..msg_type.." از قبل غیرفعال بود🔈"
				end
			end
			if matches[2] == 'عکس' then
			local msg_type = 'عکس'
			...
			if matches[2] == 'پرونده' then
			local msg_type = 'پرونده'
				if is_muted(chat_id, msg_type..': بلی') then
					savelog(msg.to.id, name_log.." ["..msg.from.id.."] set SuperGroup to: unmute "..msg_type)
					unmute(chat_id, msg_type)
					return msg_type.." باز شد🔈"
				else
			...
			local msg_type = 'همه'
				if is_muted(chat_id, msg_type..': بلی') then
					...
					return "موت  "..msg_type.." غیر فعال شده بود🔈"
				end
			end
		end


		if matches[1] == "موت کاربر" or matches[1] == "باز کردن" and is_momod(msg) then
			local chat_id = msg.to.id
			local hash = "موت کاربر"..chat_id
			local user_id = ""
			if type(msg.reply_id) ~= "nil" then
				local receiver = get_receiver(msg)
				local get_cmd = "موت کاربر"
				muteuser = get_message(msg.reply_id, get_message_callback, {receiver = receiver, get_cmd = get_cmd, msg = msg})
			elseif matches[1] == "موت کاربر" or matches[1] == "باز کردن" and string.match(matches[2], '^%d+$') then
				local user_id = matches[2]
				if is_muted_user(chat_id, user_id) then
					unmute_user(chat_id, user_id)
					savelog(msg.to.id, name_log.." ["..msg.from.id.."] , ["..user_id.."] را از لیست موت پاک کرد✅")
					return "["..user_id.."] از لیست موت ها حذف شد✅"
				elseif is_momod(msg) then
					mute_user(chat_id, user_id)
					savelog(msg.to.id, name_log.." ["..msg.from.id.."] , ["..user_id.."] را موت کرد✅")
					return "["..user_id.."] موت شد✅"
				end
			elseif matches[1] == "موت کاربر" or matches[1] == "باز کردن" and not string.match(matches[2], '^%d+$') then
				local receiver = get_receiver(msg)
				local get_cmd = "موت کاربر"
				local username = matches[2]
				local username = string.gsub(matches[2], '@', '')
		...
		if matches[1] == "لیست موت گروه" and is_momod(msg) then
			local chat_id = msg.to.id
		...
		if matches[1] == 'راهنما' and not is_owner(msg) then
			text = "✅دستور /superhelp را در پی وی ربات ارسال نمایید"
