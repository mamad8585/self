...
local function admin_user_demote(receiver, member_username, member_id)
	...
	if not data['admins'][tostring(member_id)] then
		send_large_msg(receiver, "@"..member_username..' ادمین نبود ✅')
		return
    end
	data['admins'][tostring(member_id)] = nil
	save_data(_config.moderation.data, data)
	send_large_msg(receiver, 'Admin @'..member_username..' ادمینی برکنار شد ✅')
end
...
local function username_id(cb_extra, success, result)
   ...
   local text = 'کسی به یوزرنیم @'..member..' در این گروه نیست! ✅'
   for k,v in pairs(result.members) do
      vusername = v.username
      if vusername == member then
        member_username = member
        member_id = v.peer_id
        if mod_cmd == 'ادمین' then
...
function run(msg, matches)
	...
	--[[ Experimental
	if matches[1] == 'ساخت سوپر' and matches[2] then
